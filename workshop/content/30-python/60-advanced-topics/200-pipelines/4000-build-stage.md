...
Create a new file in `cdkworkshop` called `pipeline_stage.py` with the code below:

{{<highlight python>}}
from aws_cdk import (
    core
)
from cdkworkshop_stack import CdkworkshopStack
...
Now we must add the stage to the pipeline by adding the following code to `cdkworkshop/pipeline_stack.py`:
...
## Update Requirements
For the CI/CD pipeline to contain the correct packages in the build job we need to update the `requirement.txt` file with any packages used in the repository, as shown below.
